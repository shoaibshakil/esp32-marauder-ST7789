  ...
  // menu items will be selected while scans and attacks are running
        ...
        (wifi_scan_obj.currentScanMode != WIFI_ATTACK_RICK_ROLL))
    {
      if ((wifi_scan_obj.currentScanMode == WIFI_SCAN_OFF) ||
        ...
        // Touch on right-side scroll buttons (UP / DOWN / Drag) when list exceeds screen limit
        if (pressed && current_menu && current_menu->list && current_menu->list->size() > BUTTON_SCREEN_LIMIT) {
          if (t_x >= SCREEN_WIDTH - 32) {
            int total = current_menu->list->size();
            
            // Top-right UP button tap
            if (t_y < 85) {
              if (this->menu_start_index > 0) {
                this->menu_start_index--;
                this->buildButtons(current_menu, this->menu_start_index);
                this->displayCurrentMenu(this->menu_start_index);
                delay(80);
              }
            }
            // Bottom-right DOWN button tap
            else if (t_y > SCREEN_HEIGHT - 65) {
              if (this->menu_start_index + BUTTON_SCREEN_LIMIT < total) {
                this->menu_start_index++;
                this->buildButtons(current_menu, this->menu_start_index);
                this->displayCurrentMenu(this->menu_start_index);
                delay(80);
            ...
            // Middle track tap/drag
            else {
              int track_top = 85;
              int track_bot = SCREEN_HEIGHT - 65;
              int track_h = track_bot - track_top;
              int new_start = map(constrain((int)t_y, track_top, track_bot), track_top, track_bot, 0, total - BUTTON_SCREEN_LIMIT);
              new_start = constrain(new_start, 0, total - BUTTON_SCREEN_LIMIT);
        ...
        // Individual button touch (on left/center of screen)
        if (current_menu && current_menu->list) {
          uint8_t num_visible = min((int)BUTTON_SCREEN_LIMIT, (int)(current_menu->list->size() - this->menu_start_index));
          for (uint8_t b = 0; b < num_visible; b++) {
            if (pressed && (t_x < SCREEN_WIDTH - 32 || current_menu->list->size() <= BUTTON_SCREEN_LIMIT) && display_obj.key[b].contains(t_x, t_y)) {
          ...
          for (uint8_t b = 0; b < num_visible; b++) {
            ...
            if (display_obj.key[b].justPressed()) {
              this->buttonSelected(b, idx);
            }
            if ((display_obj.key[b].justReleased()) && (!pressed)) {
              this->buttonNotSelected(b, idx);
        ...
        // Detect up, down, select in scan modes
...
void MenuFunctions::displayCurrentMenu(int start_index)
{
  ...
  if (current_menu->list != NULL)
  {
    ...
    #ifdef HAS_FULL_SCREEN
      if (current_menu->list->size() > BUTTON_SCREEN_LIMIT) {
        int total = current_menu->list->size();
        
        // Permanent UP Button on top-right
        display_obj.tft.fillRoundRect(SCREEN_WIDTH - 28, 42, 26, 36, 4, TFT_DARKGREY);
        display_obj.tft.drawRoundRect(SCREEN_WIDTH - 28, 42, 26, 36, 4, TFT_CYAN);
        display_obj.tft.fillTriangle(SCREEN_WIDTH - 15, 48, SCREEN_WIDTH - 24, 68, SCREEN_WIDTH - 6, 68, TFT_WHITE);

        // Permanent DOWN Button on bottom-right
        display_obj.tft.fillRoundRect(SCREEN_WIDTH - 28, SCREEN_HEIGHT - 52, 26, 36, 4, TFT_DARKGREY);
        display_obj.tft.drawRoundRect(SCREEN_WIDTH - 28, SCREEN_HEIGHT - 52, 26, 36, 4, TFT_CYAN);
        display_obj.tft.fillTriangle(SCREEN_WIDTH - 15, SCREEN_HEIGHT - 22, SCREEN_WIDTH - 24, SCREEN_HEIGHT - 42, SCREEN_WIDTH - 6, SCREEN_HEIGHT - 42, TFT_WHITE);

        // Vertical Track between UP and DOWN buttons
        int track_y = 82;
        int track_h = SCREEN_HEIGHT - 56 - track_y; // 182px
        display_obj.tft.fillRect(SCREEN_WIDTH - 17, track_y, 4, track_h, TFT_DARKGREY);

        // Scroll thumb
        int bar_h = max(14, track_h * BUTTON_SCREEN_LIMIT / total);
        int bar_y = track_y + ((track_h - bar_h) * start_index) / (total - BUTTON_SCREEN_LIMIT);
        display_obj.tft.fillRect(SCREEN_WIDTH - 18, bar_y, 6, bar_h, TFT_CYAN);
      }
    #endif
  }
